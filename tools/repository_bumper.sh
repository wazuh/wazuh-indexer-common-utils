...
# =========================
# Updates VERSION.json for a new version release, checks that build.gradle still
# resolves its version from it, then (depending on flags) reinitializes
...
# ====
# Assert build.gradle does not reintroduce a hardcoded version fallback.
...
# fallback can drift behind VERSION.json again, so fail rather than warn.
# ====
